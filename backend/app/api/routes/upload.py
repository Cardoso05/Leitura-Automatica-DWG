...
async def upload_project(
    ...
    if file.content_type not in [
        ...
        raise HTTPException(
            status_code=400, detail="Formato não suportado. Envie DWG ou DXF."
        )
    ...
    if file.filename.lower().endswith(".dwg"):
        ...
        if not converter.available:
            raise HTTPException(
                status_code=400,
                detail="Conversão de DWG não disponível neste ambiente. Exporte o arquivo como DXF no AutoCAD e faça o upload do .dxf.",
            )
